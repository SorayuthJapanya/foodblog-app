...
import { getOneRecipe, putRecipes } from "../api/recipeApi";
...
const EditRecipe = () => {
  ...
  useEffect(() => {
    if (recipeData) {
      ...
      setCoverImg(recipeData.coverImg);
    }
  }, [recipeData]);

  const { mutate: editRecipe } = useMutation({
    mutationFn: async ({ id, recipe }) => {
      console.log(recipe);
      const response = await putRecipes(id, recipe);
      return response.data;
    },
    onSuccess: (data) => {
      toast.success(data.success || "Edit Recipe Succesfully");
      history.back();
    },
    onError: (error) => {
      toast.error(error?.response?.data?.message || "Edit Recipe Failed");
    },
  });
